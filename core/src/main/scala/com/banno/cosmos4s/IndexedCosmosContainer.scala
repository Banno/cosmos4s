...
trait IndexedCosmosContainer[F[_], K, I, V]{
  def query(partitionKey: K, query: String, overrides: FeedOptions => FeedOptions = identity): Stream[F, V]
  def queryCustom[A: Decoder](partitionKey: K, query: String, overrides: FeedOptions => FeedOptions = identity): Stream[F, A]
  ...
  def semiInvariantFlatMap[A](f: V => F[A])(g: A => V)(implicit F: Monad[F]): IndexedCosmosContainer[F, K, I, A] = 
    new IndexedCosmosContainer.SemiInvariantFlatMap(this, f, g)
  def imapValue[A](f: V => A)(g: A => V)(implicit F: Functor[F]): IndexedCosmosContainer[F, K, I, A] = 
    new IndexedCosmosContainer.ImapValue(this, f, g)
}

object IndexedCosmosContainer {

  def impl[F[_]: ConcurrentEffect: ContextShift](container: CosmosAsyncContainer, createFeedOptions: Option[F[FeedOptions]] = None): IndexedCosmosContainer[F, String, String, Json] = 
    new BaseImpl[F](container, createFeedOptions)

  private class BaseImpl[F[_]: ConcurrentEffect: ContextShift](container: CosmosAsyncContainer, createFeedOptions: Option[F[FeedOptions]] = None)
    extends IndexedCosmosContainer[F, String, String, Json] {
      ...
      def query(partitionKey: String, query: String, overrides: FeedOptions => FeedOptions = identity): Stream[F, Json] = 
        queryCustom[Json](partitionKey, query, overrides)
      def queryCustom[A: Decoder](partitionKey: String, query: String, overrides: FeedOptions => FeedOptions = identity): Stream[F, A] = {
        Stream.eval(createFeedOptionsAlways)
          .map(overrides)
          .flatMap{options =>
            ReactorCore.fluxToStream(
              Sync[F].delay(
                container.queryItems(query, options.setPartitionKey(new PartitionKey(partitionKey)), classOf[JsonNode])
                  .byPage()
              )
            )
      ...
      def lookup(partitionKey: String, id: String): F[Option[Json]] = 
        cats.data.OptionT(
          ReactorCore.monoToEffectOpt(Sync[F].delay(
            container.readItem(id, new PartitionKey(partitionKey), new CosmosItemRequestOptions(), classOf[JsonNode])
          ))
        )
      ...
      def insert(partitionKey: String, value: Json): F[Option[Json]] = {
        cats.data.OptionT.liftF(Sync[F].delay(new CosmosItemRequestOptions()))
        .flatMap( options =>
          cats.data.OptionT(
            ReactorCore.monoToEffectOpt(Sync[F].delay(
      ...
      def delete(partitionKey: String, id: String): F[Unit] = {
        ReactorCore.monoToEffect(Sync[F].delay(
  ...
  private class MapKIndexedCosmosContainer[F[_], G[_], K, I, V](
    ...
    def query(partitionKey: K, query: String, overrides: FeedOptions => FeedOptions =  identity): Stream[G, V] =
      base.query(partitionKey, query, overrides).translate(fk)
    def queryCustom[A: Decoder](partitionKey: K, query: String, overrides: FeedOptions => FeedOptions = identity): Stream[G,A] = 
  ...
  private class ContramapPartitionKey[F[_], K, K2, I, V](
    ...
    def query(partitionKey: K2, query: String, overrides: FeedOptions => FeedOptions): Stream[F,V] = 
      base.query(contra(partitionKey), query, overrides)
    def queryCustom[A: Decoder](partitionKey: K2, query: String, overrides: FeedOptions => FeedOptions): Stream[F,A] = 
  ...
  private class ContramapId[F[_], K, I, I2, V](
    ...
    def queryCustom[A: Decoder](partitionKey: K, query: String, overrides: FeedOptions => FeedOptions): Stream[F,A] = 
  ...
  private class SemiInvariantFlatMap[F[_]: Monad, K, I, V, V2](
    ...
    def query(partitionKey: K, query: String, overrides: FeedOptions => FeedOptions): Stream[F,V2] =   
      base.query(partitionKey, query, overrides)
        .evalMap(f)
    def queryCustom[A: Decoder](partitionKey: K, query: String, overrides: FeedOptions => FeedOptions): Stream[F,A] = 
      base.queryCustom(partitionKey, query, overrides)
  ...
  implicit def partitionKey[F[_], I, V] =  new Contravariant[IndexedCosmosContainer[F,*, I, V]]{
    def contramap[A, B](fa: IndexedCosmosContainer[F,A,I,V])(f: B => A): IndexedCosmosContainer[F,B,I,V] = 
      fa.contramapPartitionKey(f)
  }

  implicit def id[F[_], K, V] = new Contravariant[IndexedCosmosContainer[F, K, *, V]]{
    def contramap[A, B](fa: IndexedCosmosContainer[F,K,A,V])(f: B => A): IndexedCosmosContainer[F,K,B,V] = 
      fa.contramapId(f)
  }
}