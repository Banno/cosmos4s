...
trait RawCosmosContainer[F[_], V]{
  ...
  def queryCustomRaw[A: Decoder](query: String,overrides: FeedOptions => FeedOptions = identity): Stream[F, A]
...
object RawCosmosContainer {
  def impl[F[_]: ConcurrentEffect: ContextShift](container: CosmosAsyncContainer, createFeedOptions: Option[F[FeedOptions]] = None): RawCosmosContainer[F,Json] = 
        ...
        Stream.eval(createFeedOptionsAlways)
          ...
          .flatMap{options =>
            ReactorCore.fluxToStream(
              Sync[F].delay(
                container.queryItems(query, options, classOf[JsonNode])
                  .byPage()
              )
            )
  ...
  private class MapKRawCosmosContainer[F[_], G[_], V](
    ...
    def queryCustomRaw[A: Decoder](query: String,overrides: FeedOptions => FeedOptions = identity): Stream[G, A] = 
  ...
  private class MapValueRawCosmosContainter[F[_], V, A](
    ...
    def queryCustomRaw[B: Decoder](query: String, overrides: FeedOptions => FeedOptions): Stream[F,B] = 
  ...
  implicit def functor[F[_]]: Functor[RawCosmosContainer[F, *]] = new Functor[RawCosmosContainer[F, *]]{
